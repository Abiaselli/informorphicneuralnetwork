...
class SinusoidalPositionalEncoding(nn.Module):
    def __init__(self, dim, max_len=seq_len):
        super().__init__()
...
class InfomorphicBlock(nn.Module):
    def __init__(self, input_dim, context_dim, hidden_dim, num_neurons):
        ...
        self.proj = nn.Linear(num_neurons, input_dim)

    def forward(self, xR, xC):
        #print(f"xr IB shape: {xR.shape}")
        #print(f"xc IB shape: {xC.shape}")

        attn_out = self.attn(xR, xC)
        ffn_out = self.ffn(attn_out, xC)
        combined = xR + self.proj(ffn_out)  # residual
        return self.norm(combined)

class InfomorphicStack(nn.Module):
    def __init__(self, num_layers, input_dim, context_dim, hidden_dim, num_neurons):
        super().__init__()
        self.blocks = nn.ModuleList([
            InfomorphicBlock(input_dim, context_dim, hidden_dim, num_neurons)
            for _ in range(num_layers)
        ])

    def forward(self, xR, xC):
        #print(f"xr stack shape: {xR.shape}")
        #print(f"xc stack shape: {xC.shape}")
        for block in self.blocks:
            xR = block(xR, xC)
        return xR

class InfomorphicEncoderDecoder(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, num_neurons):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, embed_dim)
        self.encoder = InfomorphicStack(num_layers, embed_dim, embed_dim, hidden_dim, num_neurons)
        self.decoder = InfomorphicStack(num_layers, embed_dim, embed_dim, hidden_dim, num_neurons)
        self.out_proj = nn.Linear(embed_dim, vocab_size)

    def forward(self, input_ids, decoder_input_ids):
        xR_enc = self.embed(input_ids)
        #print(f"xr enc ED shape: {xR_enc.shape}")
        context = self.encoder(xR_enc, xR_enc)  # or self-attend

        xR_dec = self.embed(decoder_input_ids)
        xC = context.mean(dim=1).unsqueeze(1).expand_as(xR_dec)  # crude global context
        decoder_out = self.decoder(xR_dec, xC)

        return self.out_proj(decoder_out)

class InfomorphicTransformer(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, num_neurons, max_len=seq_len):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, embed_dim)
    ...
    def forward(self, input_ids, decoder_input_ids, causal_mask=None):
        # Embedding + Positional
        xR_enc = self.pos(self.embed(input_ids))          # (B, S, D)
        xR_dec = self.pos(self.embed(decoder_input_ids))  # (B, T, D)

        enc_context = self.encoder(xR_enc, xR_enc)  # global context for now

        if causal_mask is not None:
            xR_dec = xR_dec.masked_fill(causal_mask == 0, 0)
        xR_dec = xR_dec.mean(dim=1).unsqueeze(1).expand_as(enc_context)
        dec_out = self.decoder(xR_dec, enc_context)
        return self.out_proj(dec_out)  # (B, T, vocab)

class MemoryModule(nn.Module):
    def __init__(self, key_dim, value_dim, max_memory=512):
        super().__init__()
    ...
    def retrieve(self, query, top_k=1):
        ...
        sims = [F.cosine_similarity(query, k.unsqueeze(0)) for k in self.keys]
        top_idx = torch.topk(torch.stack(sims), top_k).indices
        return torch.stack([self.values[i] for i in top_idx]).mean(dim=0)

class PlasticMemoryInfomorphicLayer(nn.Module):
    def __init__(self, input_dim_R, input_dim_C, memory_dim, num_neurons, alpha=0.3, decay=0.1):
        super().__init__()
        self.num_neurons = num_neurons
        self.alpha = alpha
        self.decay = decay
        ...
        # Long-term weights
        self.wR = nn.Parameter(torch.randn(num_neurons, input_dim_R) * 0.01)
        self.wM = nn.Parameter(torch.randn(num_neurons, memory_dim) * 0.01)
        self.bias = nn.Parameter(torch.zeros(num_neurons))

        # Plastic weights (short-term)
        self.register_buffer("plastic_R", torch.zeros(num_neurons, input_dim_R))
        self.register_buffer("plastic_M", torch.zeros(num_neurons, memory_dim))

        # Recurrent memory trace (shared across batch)
        self.register_buffer("memory_trace", torch.zeros(1, memory_dim))
    ...
    def forward(self, xR, xC):
        """
        xR: (batch, input_dim_R)
        Returns: (batch, num_neurons)
        """
        batch_size, len, _ = xR.size()
        mem = self.memory_trace.expand(-1, len)  # context memory

        eff_wR = self.wR + self.alpha * self.plastic_R
        eff_wM = self.wM + self.alpha * self.plastic_M

        r = F.linear(xR, eff_wR)  # (batch, neurons)
        c = F.linear(xC, self.wC) + self.bias_C

        m = F.linear(mem, eff_wM)
        r = torch.einsum('bse, ace->bse', c, r)
        r = torch.einsum('be, ace->ace', m, r)
        A = r * (0.5 + torch.sigmoid(2 * r))
        prob = torch.sigmoid(A)

        # === Hebbian update + memory trace ===
        with torch.no_grad():
            y = (prob > 0.5).float()
            self.memory_trace = 0.95 * self.memory_trace + 0.05 * y.mean(dim=0)


            hebb_R = torch.einsum('bci,bcj->ij', y, xR) / batch_size
            hebb_M = torch.einsum('bci,cj->ij', y, mem) / batch_size

            self.plastic_R = (1 - self.decay) * self.plastic_R + hebb_R
            self.plastic_M = (1 - self.decay) * self.plastic_M + hebb_M

        return prob


class PlasticMemoryBlock(nn.Module):
    def __init__(self, input_dim, memory_dim, context_dim, hidden_dim, num_neurons):
        super().__init__()
        self.attn = PlasticMemoryInfomorphicLayer(input_dim, context_dim, hidden_dim, memory_dim, num_neurons)
        self.ffn = PlasticMemoryInfomorphicLayer(num_neurons, context_dim, hidden_dim, memory_dim, num_neurons)
        self.proj = nn.Linear(num_neurons, input_dim)
...
class GatedPlasticMemoryBlock(nn.Module):
    def __init__(self, input_dim, memory_dim, context_dim, hidden_dim, num_neurons):
        super().__init__()
        self.block = PlasticMemoryBlock(input_dim, memory_dim, context_dim, hidden_dim, num_neurons)
    ...
    def forward(self, xR, xC):
        gate_val = self.gate(xR).mean(dim=1, keepdim=True)  # (batch, 1)
        gated_xR = gate_val * self.block(xR, xC) + (1 - gate_val) * xR  # residual if blocked
        return gated_xR

class PlasticMemoryStack(nn.Module):
    def __init__(self, num_layers, input_dim, context_dim, hidden_dim, num_neurons):
        super().__init__()
        self.blocks = nn.ModuleList([
            GatedPlasticMemoryBlock(input_dim, memory_dim=input_dim, context_dim=context_dim, hidden_dim=hidden_dim, num_neurons=num_neurons)
            for _ in range(num_layers)
        ])
...
class MemoryInfomorphicTransformer(nn.Module):
    def __init__(self, vocab_size, embed_dim, context_dim, hidden_dim, num_layers, num_neurons, max_len=seq_len):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, embed_dim)
        self.pos = SinusoidalPositionalEncoding(embed_dim, max_len)

        self.encoder = PlasticMemoryStack(num_layers, embed_dim, context_dim, hidden_dim, num_neurons)
        self.decoder = PlasticMemoryStack(num_layers, embed_dim, context_dim, hidden_dim, num_neurons)
...
def generate_causal_mask(seq_len, device):
    return torch.tril(torch.ones(seq_len, seq_len)).unsqueeze(0).to(device)  # shape (1, T, T)
...
def train_model(model, dataloader, val_loader, optimizer, criterion, device):
    ...
    for batch_idx, (src, target) in enumerate(dataloader):
        ...
        val_dec_input, val_tgt = prepare_decoder_input_and_target(val_tgt)

        val_preds = model(val_src, val_dec_input)
...
def train_model_causal(model, dataloader, val_loader, optimizer, criterion, device):
    model.train()
    total_loss = 0
    n = 0
    ...
    for batch_idx, (src, target) in enumerate(dataloader):
        ...
        model = model.to(device)
        # 🔹 Get predictions & rule-modified embeddings
        causal_mask = generate_causal_mask(decoder_input.size(1), device)

        output = model(src, decoder_input, causal_mask).to(device)
        # 🔹 Ensure `output` and `target_labels` have the same sequence length
        seq_len = min(output.shape[1], target_labels.shape[1])  # Get the shorter sequence length
        output = output[:, :seq_len, :]  # Truncate logits if too long
        target_labels = target_labels[:, :seq_len]  # Truncate targets if too long
        ...
        causal_mask = generate_causal_mask(val_dec_input.size(1), device)

        val_preds = model(val_src, val_dec_input, causal_mask)
        val_loss = criterion(val_preds.view(-1, val_preds.size(-1)), val_tgt.view(-1)).item()
...
def main():
    ...
    model = MemoryInfomorphicTransformer(vocab_size, embed_dim=d_model, context_dim=context_dim, hidden_dim=hidden_d, num_layers=n_layers, num_neurons=n_neurons).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        avg_loss = train_model_causal(model, dataloader, val_loader, optimizer, criterion, device)
