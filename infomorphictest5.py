...
class PlasticMemoryInfomorphicLayer(nn.Module):
    def __init__(self, num_neurons, input_dim_R, memory_dim, alpha=0.3, decay=0.1):
        super().__init__()
        self.num_neurons = num_neurons
        self.alpha = alpha
        self.decay = decay
        ...
        # Recurrent memory trace (one per batch item)
    ...
    def forward(self, xR):
        ...
        Returns:
            prob: (batch, num_neurons)
        """
        batch_size = xR.size(0)
        mem = self.memory_trace.expand(batch_size, -1)  # same memory across batch
        ...
        A = r * (0.5 + torch.sigmoid(2 * r * m))
        prob = torch.sigmoid(A)

        # === Hebbian and Memory Update ===
        with torch.no_grad():
            y = (prob > 0.5).float()
            self.memory_trace = 0.95 * self.memory_trace + 0.05 * y.mean(dim=0)

            hebb_R = torch.einsum('bi,bj->ij', y, xR) / batch_size
            hebb_M = torch.einsum('bi,bj->ij', y, mem) / batch_size
...
class PlasticMemoryInfomorphicLayer(nn.Module):
    ...
    def forward(self, xR):
        ...
        """
        batch_size = xR.size(0)
        mem = self.memory_trace.expand(batch_size, -1)  # context memory
...
class PlasticMemoryBlock(nn.Module):
    def __init__(self, input_dim, memory_dim, hidden_dim, num_neurons):
        super().__init__()
        self.attn = PlasticMemoryInfomorphicLayer(num_neurons, input_dim, memory_dim)
        self.ffn = PlasticMemoryInfomorphicLayer(num_neurons, num_neurons, memory_dim)
        self.proj = nn.Linear(num_neurons, input_dim)
...
class PlasticMemoryStack(nn.Module):
    def __init__(self, num_layers, input_dim, hidden_dim, num_neurons):
        super().__init__()
        self.blocks = nn.ModuleList([
            PlasticMemoryBlock(input_dim, memory_dim=input_dim, hidden_dim=hidden_dim, num_neurons=num_neurons)
            for _ in range(num_layers)
        ])
...
class MemoryInfomorphicTransformer(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, num_neurons, max_len=seq_len):
        ...
        self.encoder = PlasticMemoryStack(num_layers, embed_dim, hidden_dim, num_neurons)
        self.decoder = PlasticMemoryStack(num_layers, embed_dim, hidden_dim, num_neurons)
...
def main():
    ...
    model = MemoryInfomorphicTransformer(vocab_size, embed_dim=d_model, hidden_dim=hidden_d, num_layers=n_layers, num_neurons=n_neurons).to(device)
